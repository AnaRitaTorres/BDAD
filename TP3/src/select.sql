...
/*1.LISTAR CLIENTES POR ORDEM  ALFABETICA - permite a verificação do trigger 1*/
.width 20 25 8
SELECT DISTINCT PESSOA.nome AS Nome, CLIENTE.necessidadesEspeciais AS Necessidades_Especiais, CLIENTE.desconto AS Desconto
FROM PESSOA 
INNER JOIN CLIENTE 
...
/*2.SALARIO MEDIO DOS FUNCIONARIOS POR FUNCAO DESEMPENHADA - permite a verificação dos triggers 2 e 3*/
.width 20 12
...
/*9.NUMERO DE LUGARES DE UM COMBOIO*/
.width 10
...
/*10.BILHETES COMPRADOS NUM DETERMINADO DIA NUMA ESTACAO ESPECIFICA*/
.width 11 5 28
SELECT BILHETE.dataCompra AS Data_Compra,BILHETE.preco AS Preco, ESTACAO.nome AS Estacao_Compra
FROM BILHETE
INNER JOIN ITINERARIOESTACAO
ON BILHETE.idItinerario=ITINERARIOESTACAO.idItinerario
INNER JOIN ESTACAO 
ON ESTACAO.idEstacao=ITINERARIOESTACAO.idEstacao
ORDER BY BILHETE.dataCompra;
